...
def joinFactors(factors):
    ...
    for factor in factors:
        for conditionedVariable in factor.conditionedVariables():
            joinedConditionedVariables.add(conditionedVariable)
        for unconditionedVariable in factor.unconditionedVariables():
            joinedUnconditionedVariables.add(unconditionedVariable)
    ...
    for assignment in joinedFactor.getAllPossibleAssignmentDicts():
        assignmentProduct = 1
        for factor in factors:
            assignmentProduct = assignmentProduct * Factor.getProbability(factor, assignment)
        Factor.setProbability(joinedFactor, assignment, assignmentProduct)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        reducedFactor = Factor(reducedUnconditionedVariables, factor.conditionedVariables(),
                               factor.variableDomainsDict())
...
def normalize(factor):
    ...
    for assignment in factor.getAllPossibleAssignmentDicts():
        initSumProbs += factor.getProbability(assignment)
