    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                if assignment[FOOD_HOUSE_VAR] == calcPosValue(obsPos[0], obsPos[1]):
                    red_probability = PROB_FOOD_RED
                    blue_probability = 1 - PROB_FOOD_RED
                    neither_probability = 0
                elif assignment[GHOST_HOUSE_VAR] == calcPosValue(obsPos[0], obsPos[1]):
                    red_probability = PROB_GHOST_RED
                    blue_probability = 1 - PROB_GHOST_RED
                    neither_probability = 0
                else:
                    red_probability = 0
                    blue_probability = 0
                ...
                if assignment[obsVar] == BLUE_OBS_VAL:
                    obsFactor.setProbability(assignment, blue_probability)
                elif assignment[obsVar] == RED_OBS_VAL:
                    obsFactor.setProbability(assignment, red_probability)
                else:
                    obsFactor.setProbability(assignment, neither_probability)
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        leftExpectedValue = CPT.getProbability(foodTopLeft) * WON_GAME_REWARD + CPT.getProbability(foodTopRight) * GHOST_COLLISION_REWARD
        rightExpectedValue = CPT.getProbability(foodTopLeft) * GHOST_COLLISION_REWARD + CPT.getProbability(foodTopRight) * WON_GAME_REWARD
