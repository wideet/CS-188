...
class Graph(object):
    ...
    def __init__(self, variables):
        ...
        for var in variables:
            ...
            self.inputs[var] = var.get_parents()
            self.outputs[var] = var.forward(var.get_parents())
            self.gradients[var] = np.zeros_like(self.outputs[var])
    ...
    def add(self, node):
        ...
        parent_outputs = [self.outputs[parent] for parent in node.get_parents()]
        self.inputs[node] = parent_outputs
        self.outputs[node] = node.forward(parent_outputs)
    ...
    def backprop(self):
        ...
        for node in reversed(self.nodes):
            updated_gradients = node.backward(self.inputs[node], self.gradients[node])
            p_count = 0
            for parent in node.get_parents():
                self.gradients[parent] = self.gradients[parent] + updated_gradients[p_count]
                p_count += 1
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        temp_mat = 0.5 * (inputs[0] - inputs[1]) ** 2
        return np.mean(temp_mat)
    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
