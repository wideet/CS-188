...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        hidden_layer = 50
        self.w1 = nn.Variable(1, hidden_layer)
        self.w2 = nn.Variable(hidden_layer, 1)
    ...
    def run(self, x, y=None):
        ...
        xm_plus_b1 = nn.MatrixVectorAdd(graph, xw1, self.b1)
        reLU_xm_plus_b1 = nn.ReLU(graph, xm_plus_b1)
        reLU_xm_plus_b1_w2 = nn.MatrixMultiply(graph, reLU_xm_plus_b1, self.w2)
        reLU_xm_plus_b1_w2_plus_b2 = nn.MatrixVectorAdd(graph, reLU_xm_plus_b1_w2, self.b2)
        ...
        if y is not None:
            ...
            loss = nn.SquareLoss(graph, reLU_xm_plus_b1_w2_plus_b2, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            return graph.get_output(reLU_xm_plus_b1_w2_plus_b2)
...
class OddRegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        reLU_xm_plus_b1_w2 = nn.MatrixMultiply(graph, reLU_xm_plus_b1, self.w2)
        reLU_xm_plus_b1_w2_plus_b2 = nn.MatrixVectorAdd(graph, reLU_xm_plus_b1_w2, self.b2)
        # print graph.get_output(reLU_xm_plus_b1_w2_plus_b2)

        neg_x = x * -1
        neg_input_x = nn.Input(graph, neg_x)
        neg_xw1 = nn.MatrixMultiply(graph, neg_input_x, self.w1)
        neg_xm_plus_b1 = nn.MatrixVectorAdd(graph, neg_xw1, self.b1)
        neg_reLU_xm_plus_b1 = nn.ReLU(graph, neg_xm_plus_b1)
        neg_reLU_xm_plus_b1_w2 = nn.MatrixMultiply(graph, neg_reLU_xm_plus_b1, self.w2)
        neg_reLU_xm_plus_b1_w2_plus_b2 = nn.MatrixVectorAdd(graph, neg_reLU_xm_plus_b1_w2, self.b2)
        # print graph.get_output(neg_reLU_xm_plus_b1_w2_plus_b2)

        neg_multiplier_input = nn.Input(graph, np.ones((1, 1)) * -1.0)
        neg_gx = nn.MatrixMultiply(graph, neg_reLU_xm_plus_b1_w2_plus_b2, neg_multiplier_input)
        # print graph.get_output(neg_gx)
        fx = nn.MatrixVectorAdd(graph, reLU_xm_plus_b1_w2_plus_b2, neg_gx)
...
class DigitClassificationModel(Model):
    ...
    def __init__(self):
        ...
        self.w2 = nn.Variable(hidden_layers, 10)
        # self.w3 = nn.Variable(hidden_layers, 10)
    ...
    def run(self, x, y=None):
        ...
        reLU_xm_plus_b1_w2 = nn.MatrixMultiply(graph, reLU_xm_plus_b1, self.w2)
        reLU_xm_plus_b1_w2_plus_b2 = nn.MatrixVectorAdd(graph, reLU_xm_plus_b1_w2, self.b2)
        # reLU_l2 = nn.ReLU(graph, reLU_xm_plus_b1_w2_plus_b2)
        # l2_times_w3 = nn.MatrixMultiply(graph, reLU_l2, self.w3)
        # l2_times_w3_plus_b3 = nn.MatrixVectorAdd(graph, l2_times_w3, self.b3)

        if y is not None:
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss = nn.SoftmaxLoss(graph, reLU_xm_plus_b1_w2_plus_b2, input_y)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(reLU_xm_plus_b1_w2_plus_b2)
...
class LanguageIDModel(Model):
    ...
    def __init__(self):
        ...
        self.h0 = nn.Variable(self.hidden_layer)

        self.w1 = nn.Variable(47, self.hidden_layer)
        self.w2 = nn.Variable(self.hidden_layer, self.hidden_layer)
        self.b1 = nn.Variable(self.hidden_layer)
        self.b2 = nn.Variable(self.hidden_layer)
        self.w3 = nn.Variable(self.hidden_layer, 5)
    ...
    def run(self, xs, y=None):
        ...
        "*** YOUR CODE HERE ***"
        graph = nn.Graph([self.w1, self.b1, self.w2, self.b2, self.h0, self.w3])
        HO_Input = nn.Input(graph, np.zeros((batch_size, self.hidden_layer)))
        H = nn.MatrixVectorAdd(graph, HO_Input, self.h0)

        for letter in xs:
            input_x = nn.Input(graph, letter)
            xw1 = nn.MatrixMultiply(graph, input_x, self.w1)
            xm_plus_b1 = nn.MatrixVectorAdd(graph, xw1, self.b1)
            plus_H0 = nn.Add(graph, xm_plus_b1, H)
            reLU_xm_plus_b1 = nn.ReLU(graph, plus_H0)
            reLU_xm_plus_b1_w2 = nn.MatrixMultiply(graph, reLU_xm_plus_b1, self.w2)
            reLU_xm_plus_b1_w2_plus_b2 = nn.MatrixVectorAdd(graph, reLU_xm_plus_b1_w2, self.b2)
            input_0 = nn.Input(graph, np.zeros_like(reLU_xm_plus_b1_w2_plus_b2, dtype=float))
            H = nn.MatrixVectorAdd(graph, reLU_xm_plus_b1_w2_plus_b2, input_0)

        output = nn.MatrixMultiply(graph, H, self.w3)

        if y is not None:
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss = nn.SoftmaxLoss(graph, output, input_y)
            return graph
        else:
            "*** YOUR CODE HERE ***"
