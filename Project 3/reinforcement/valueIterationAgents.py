            ...
            for state in self.mdp.getStates():
                ...
                for action in self.mdp.getPossibleActions(state):
                    ...
                    for nextState, prob in self.mdp.getTransitionStatesAndProbs(state, action):
                        currVal += prob * (self.mdp.getReward(state, action, nextState) + self.discount * currValues[nextState])
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            for action in self.mdp.getPossibleActions(state):
                currVal = 0
                for nextState, prob in self.mdp.getTransitionStatesAndProbs(state, action):
                    currVal += prob * (self.mdp.getReward(state, action, nextState) + self.discount * self.values[nextState])
                maxVal = max(maxVal, currVal)
            self.values[state] = maxVal
